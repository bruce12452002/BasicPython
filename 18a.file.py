...
# 常用的為 r w a r+ w+ a+
# r 可讀不可寫； w 可寫不可讀，寫的時候會先將檔案清空； a 可寫不可讀，寫的時候從最後增加
# open("test.txt", encoding="UTF-8")，mode 不設定預設是 rt，也就是讀取文字檔，但是不能給空字串
# 沒有什麼 rw，如果讀寫都要，可用 +，18b會說明
...
write_file.flush()
write_file.close() # close 也會先執行 flush
...
print("============ 用不用 flush 的差異 ============")
# 強制寫入檔案，不用等到 close 時才寫入檔案
# 此例每2秒 flush 寫入一次，沒加會全部執行完才寫入
import time
